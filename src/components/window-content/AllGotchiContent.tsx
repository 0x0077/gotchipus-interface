          ...
          <div className="win98-group-box">
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  <span>🔍</span>
                  <span>{t('allGotchi.filtersSearch')}</span>
      ...
      <div className={`flex gap-4 flex-1 overflow-hidden ${isMobileMode ? 'flex-col' : 'p-4'}`}>
        ...
        <div className="bg-win98-face border-2 border-[#808080] shadow-win98-outer p-3">
          ...
          {searchId && !isSearching && (
            <button
              ...
              className="mt-2 w-full px-3 py-1 bg-win98-face border-2 border-white border-r-[#808080] border-b-[#808080] active:border-r-white active:border-b-white active:border-l-[#808080] active:border-t-[#808080] text-xs"
            >
        ...
        <div className="bg-win98-face border-2 border-[#808080] shadow-win98-outer p-3">
          <div className="flex justify-between items-center mb-3">
            <h3 className="text-sm font-bold text-[#000080]">{t('common.filters')}</h3>
            <button
        ...
        <div className="bg-win98-face border-2 border-[#808080] shadow-win98-inner p-2">
          <div className="text-xs text-[#808080] text-center">
            {t('allGotchi.showing', { count: allMetadata.length, total: TOTAL_NFTS.toLocaleString() })}
          </div>
        </div>
          </div>
        )}

      <div ref={scrollContainerRef} className={`flex-1 bg-win98-face overflow-auto scrollbar-hide ${isMobileMode ? '' : ''}`}>
        <div className={isMobileMode ? 'p-2' : 'p-4'}>
          {!isMobileMode && (
            <div className="mb-2 px-1 mt-2">
              <div className="win98-group-box">
                <div className="win98-group-title text-xs font-bold text-[#000080]">{t('allGotchi.collectionTitle')}</div>
          ...
          {error ? (
            <div className="text-center border-2 border-[#808080] shadow-win98-inner bg-[#d4d0c8] py-16">
              <p className="font-bold text-[#cc0000] text-lg mb-2">⚠️ {t('allGotchi.serviceUnavailable')}</p>
              <p className="text-[#808080] text-sm mb-1">{t('allGotchi.serviceUnavailableDesc')}</p>
            </div>
          ) : isLoading && allMetadata.length === 0 ? (
            <div className="flex items-center justify-center py-20">
              <Win98Loading text={t('allGotchi.loadingGotchi')} />
            </div>
          ) : allMetadata.length === 0 ? (
            <div className="text-center border-2 border-[#808080] shadow-win98-inner bg-[#d4d0c8] py-16">
              <p className="font-bold text-[#808080] text-lg mb-2">{t('allGotchi.noNfts')}</p>
              <p className="text-[#808080] text-sm">{t('allGotchi.noNftsDesc')}</p>
            </div>
          ) : (
          ...
          {hasMore && allMetadata.length > 0 && (
            ...
            >
              {isLoading ? (
                <div className="text-center bg-win98-face border-2 border-[#808080] shadow-win98-outer p-4">
                  <Win98Loading text="Loading..." />
                  <p className="mt-2 text-xs">{t('allGotchi.loadingMore')}</p>
                </div>
              ) : (
